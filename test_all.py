...
# 一级网址
url = 'http://www.sses.sh.cn/shhjkxw/eiareport/action/eia_eiaZXDetailList.do'
...
# 网站显示一共有 1036 页
num0 =0
for page in range(901,1037):
    num0 += 1
    print(num0)
    get_urls(url, page)

# 把所有的二级网站 存成文本
with open('urls_all','w') as f:
...
df2 = pd.DataFrame({ '发布日期' : pd.Categorical(issue_date_sum),
                     ...
                     '建设单位联系方式':pd.Series(company_phone_sum,)
                     })


df2.to_excel('shanghai_901_1036.xlsx','901-1036')
